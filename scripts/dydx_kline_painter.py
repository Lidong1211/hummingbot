...
class DydxKlinePainter(StrategyV2Base):
    """
    dYdX v4 合约画线 / 自成交打点脚本 (Kline Painter)
    1. 实时监听本地 dYdX 盘口深度与外部参考价格行情；
    2. 定时根据目标点位以 Taker 方式吃掉盘口挂单，生成连续真实的 K 线成交记录与交易量；
    3. 内置仓位重平衡机制，防止单边累积过多敞口。
    """

    def __init__(self, connectors: Dict[str, ConnectorBase], config: DydxKlinePainterConfig):
    ...
    def _execute_kline_paint(self):
        ...
        if best_bid.is_nan() or best_ask.is_nan() or target_price is None or target_price.is_nan():
            self.logger().warning("[PAINTER] 盘口或目标行情数据未就绪，跳过本次打点。")
            return

        self.logger().info(
            f"[PAINTER] 盘口状态: BestBid={best_bid:.4f}, BestAsk={best_ask:.4f}, TargetPrice={target_price:.4f} | "
            f"当前净持仓={self._current_inventory:.2f}"
        )

        trade_amount = taker_conn.quantize_order_amount(self.config.trading_pair, self.config.trade_amount)

        # 1. 优先仓位重平衡 (如果净多头或净空头超过阈值，反向吃单平仓)
        if abs(self._current_inventory) >= self.config.max_inventory_limit:
            if self._current_inventory > 0:
                self.logger().info(f"[PAINTER REBALANCE] 仓位过多 ({self._current_inventory}) -> 主动卖出平仓 {trade_amount}...")
                self._submit_taker_order(TradeType.SELL, best_bid, trade_amount)
            else:
                self.logger().info(f"[PAINTER REBALANCE] 空头过多 ({self._current_inventory}) -> 主动买入平仓 {trade_amount}...")
        ...
        self.logger().info(
            f"[PAINTER EXECUTE] >>> 发起画线成交: {trade_type.name} {trade_amount} @ {order_price} "
    ...
    def _submit_taker_order(self, trade_type: TradeType, price: Decimal, amount: Decimal):
        ...
        try:
            ...
            self.logger().error(f"[PAINTER ERROR] 下单异常: {str(e)}", exc_info=True)
    ...
    def did_fill_order(self, event: OrderFilledEvent):
        ...
        msg = (
            f"[PAINTER SUCCESS] ✅ 成交完成! {event.trade_type.name} {event.amount} {event.trading_pair} @ "
    ...
    def did_fail_order(self, event: MarketOrderFailureEvent):
        self.logger().error(f"[PAINTER FAILED] ❌ 画线订单失败: {event.order_id}, 类型: {event.order_type}")
